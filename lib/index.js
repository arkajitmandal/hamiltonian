...
function hamiltonian(basis, m = 1, points = 100) {
  ...
  for (var i = 0; i < N; i++) {
    for (var j = 0; j < N; j++) {
      ...
      H[i][j] = -basis.Dij(i, j, false, points) / m;
    }
  }
  return H;
}

function getV(Vx, basis, points = 100) {
  let N = basis.set.length;
  let V = [];
  for (var a = 0; a < N; a++) {
    V.push([]);
  ...
  let dx = (basis.upperLimit - basis.lowerLimit) / points;
  for (var i = 0; i < N; i++) {
    for (var j = 0; j < N; j++) {
      let psi1 = basis.set[i];
      let psi2 = basis.set[j];
      let total = 0.0;
      for (var k = 0; k < points; k++) {
        let x = basis.set[0].lowerLimit + k * dx;
        let vx1 = math.eval(Vx, { x: x });
        let vx2 = math.eval(Vx, { x: x + dx });
        total +=
          dx /
          2 *
          (psi1.at(x) * psi2.at(x) * vx1 + psi1.at(x + dx) * psi2.at(x + dx) * vx2);
      }
      V[i][j] = total;
    }
  }
  ...
  if (name === 'legendre') {
    ...
    for (var l = 0; l < N; l++) {
      ...
      bfx[l] =
        ...
        L.toString() +
        ') )';
      fx.push(new BasisFunction(bfx[l], up, down));
    }
    let fourier = new Basis(fx);
    fourier.orthoNormalize();
    return fourier;
  }
}
