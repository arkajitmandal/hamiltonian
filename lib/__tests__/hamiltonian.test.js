...
describe('Hamiltonian Matrix without V', () => {
  it('Creation of a Hamiltonian Matrix', () => {
    ...
    let basis = new hamiltonian.Basis([fx1, fx2]);
    basis.orthoNormalize();
    let H = hamiltonian.hamiltonian(basis);
    assert(H.length === basis.set.length, 'Hamiltonian Dimention not correct');
  });
});
describe('Potential Matrix', () => {
  it('Creation of a Potential Matrix', () => {
    let fx1 = new hamiltonian.BasisFunction('sin(x)');
    let fx2 = new hamiltonian.BasisFunction('cos(x)');
    let basis = new hamiltonian.Basis([fx1, fx2]);
    basis.orthoNormalize();
    let V = hamiltonian.getV('x^2', basis);
    assert(V.length === basis.set.length, 'Hamiltonian Dimention not correct');
  });
  console.log(hamiltonian.commonBasis('fourier-sin', 4));
});
